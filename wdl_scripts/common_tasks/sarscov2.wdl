...
task nextclade_one_sample_v0 {
    ...
    input {
        ...
        String docker = "cr.yandex/crp0pd4faec3e1qvam30/nextclade:0.14.4"
    }
...
task nextclade_one_sample {
    ...
    input {
        ...
        File qc_config_json
        File gene_map
        #String docker = "cr.yandex/crp0pd4faec3e1qvam30/nextclade:1.2.3"
...
task nextclade_python {
    meta {
        description: "Nextclade classification of one sample. Leaving optional inputs unspecified will use SARS-CoV-2 defaults."
    }
    input {
        File  nextclade_tsv
        String docker = "cr.yandex/crp0pd4faec3e1qvam30/python:3.8.11-buster"
    }
    command <<<
        set -e
        cp "~{nextclade_tsv}" input.tsv
        python3 <<CODE
        # transpose table
        import codecs
        with codecs.open('input.tsv', 'r', encoding='utf-8') as inf:
...
task sequencing_report {
    ...
    input {
        ...
        String  docker = "cr.yandex/crp0pd4faec3e1qvam30/sc2-rmd:0.1.20"
    }
    command {
        set -e
        /docker/reports.py \
...
task sc2_meta_final {
    ...
    input {
        ...
        String        docker = "cr.yandex/crp0pd4faec3e1qvam30/py3-bio:0.1.2"
    }
...
task gisaid_uploader {
  ...
  runtime {
    docker: "cr.yandex/crp0pd4faec3e1qvam30/gisaid-cli:1.0"
    memory: "2 GB"
    cpu: 2
    disks: "local-disk 100 HDD"
  }
